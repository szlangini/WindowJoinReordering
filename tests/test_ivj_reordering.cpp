...
TEST(JoinReorderingTest,
     ReorderingValidation_IntervalJoin_Case_ET_even_bounds) {
...
// IntervalJoin with unequal upper and lower bounds
TEST(JoinReorderingTest,
     ReorderingValidation_IntervalJoin_Case_ET_uneven_bounds) {
  // Use the helper function to create streams A, B, C
  auto A = createStream("A", 50, linearValueDistribution, 100, 1);
  auto B = createStream("B", 50, linearValueDistribution, 100, 2);
  auto C = createStream("C", 50, linearValueDistribution, 100, 3);
